...
| `03_tools` | `src/platform/tools`、`src/integrations/mcp` | StockMind 工具白名单与受控草稿；MCP 只读 Server（`app/mcp/server.py`） | Schema、权限、超时、审计；MCP 白名单不变量 | MCP 只读已实现（V1.1）/ Mule Bridge 规划 |
