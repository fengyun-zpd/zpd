...
V1 预置补货策略、安全库存、仓库特殊规则、供应商约束和到货异常 SOP 五类文档。用户明确需要补充资料时，`admin` 可经由受控 REST/API 页面导入单篇不超过 80,000 字符的 Markdown/TXT 证据文本；服务按段落分块并写入 FTS，Embedding 可用时附加向量。导入采用幂等操作、内容摘要去重和审计，但导入文档不触发规则解析/发布，始终不能直接进入补货计算。文档编辑、启停、回滚、结构化规则发布与 Cross-Encoder 重排仍放 V1.1。
...
适配器提供正常成功、明确失败、超时但成功、重复幂等请求和查询订单状态接口。故障模式由 `admin` 页面/REST 切换并审计，不能由 Agent 修改。`buyer` 或 `admin` 可手动查询其有权限的未知订单，`system` 可执行后台恢复；二者调用同一状态恢复服务。未知状态恢复必须沿用原外部幂等键，先查询再决定状态迁移，禁止无依据重试。
...
### 12.1 模块化目标架构（V2/V3，规划中）

V1 的单 Agent 和确定性补货服务继续作为稳定基线；后续新项目按平台域与业务插件拆分：

```text
SupervisorGraph
  ├─ 领域子 Agent（订单/物流/政策/退款，工具白名单）
  ├─ 确定性领域服务（资格、金额、状态、幂等、权限）
  ├─ RAG 与记忆（pgvector 基线，Graphiti/Neo4j 可选）
  ├─ MCP 工具层（Function Calling 兼容适配）
  └─ 评测/观测/回退/人工审批平台
```

售后业务插件的写入能力必须经过 `TenantContext`、能力矩阵、审批和幂等门禁；CrewAI 只作为可选子流程编排器，Mule Agent Bridge 只作为外部 Agent 网络适配器。完整拆分、微调、Graphiti/Neo4j 和 Mule 集成均未纳入 V1 已实现范围，详见 [`docs/`](docs/README.md) 与 [`docs/adr/`](docs/adr/README.md)。
...
- V4.6（2026-09-07）：补充会话身份与归属校验：`X-Actor-Id` 是创建会话的唯一身份事实源，请求体身份不一致时拒绝；消息读取和发送均校验 `thread_id` 归属，防止跨用户会话访问，并以集成测试固定该边界。

- V4.5（2026-09-07）：采购权限矩阵下沉领域服务并与 API 对齐，`admin` 可执行采购状态命令且审计保留真实操作者；新增 liveness `/health` 与 Agent 就绪 `/ready` 的能力分层及回归验证。

- 模块化演进基线（2026-09-04）：登记新项目远程 `fengyun-zpd/dianshang-shouhou`，新增平台/业务插件分层、LangGraph supervisor、可选 CrewAI 子流程、MCP/Mule Agent Bridge、RAG/记忆、模型能力矩阵与回退门禁。该架构为 V2/V3 规划，不改变 V1 单 Agent、确定性领域服务和人工审批边界。

- V4.4 知识资料导入（2026-09-02）：补充受控 RAG 资料入口：`admin` 通过 API/页面导入 Markdown/TXT 证据，服务分块建立 FTS、可选 Embedding，使用规范化内容哈希、幂等操作和审计防止重复。导入内容不经规则解析器，不改变结构化规则或任何确定性计算输入；新增集成与浏览器回归测试。
